...
class Day7_P2 {
	...
	public static String next(HashMap<Character, ArrayList<Character>> map, int size, String s) {
		if (s.length() == size + 4) {
			...
			for (char next : map.keySet()) {
				...
				//System.out.println("Is " + s.charAt(s.length() -1) + " in " + temp);
				for (char c : temp) {
					if (!s.contains(Character.toString(c))) {
						ready = false;
					}
				}
				//System.out.println("\tIs next? " + next);
	...
	public static void main(String[] args) {
		try {
			...
			//printMap(bla);System.out.println();
			...
			for (int i = 0; i < steps.length(); i += 2) {
				...
				if (!map.containsKey(test)) {
					for (ArrayList<Character> temp : map.values()) {
						/*if (temp.size() == 1 && temp.contains(test)) {
							instr += test;
							fInd = avail.size() - 1;
							System.out.println("\t instr = " + test);
							break;
						}*/
						if (!avail.contains(test) && temp.contains(test)) {
							avail.add(test);
							//System.out.println("\t avail += " + test);
						}
					}
				}
			}
			...
			Collections.sort(avail);
			instr += avail.get(0);
			//System.out.println("First: " + instr);
			avail.remove(0);
